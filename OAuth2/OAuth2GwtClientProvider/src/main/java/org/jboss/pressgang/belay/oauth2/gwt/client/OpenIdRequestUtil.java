 ...
 * Belay user management endpoints. AuthorizationRequests are used with these user management endpoints if they return
...
public class OpenIdRequestUtil {
    ...
    /**
     *
     * @param url URL of the OAuth 2.0 Authorization Server's endpoint to associate a new identity with a user
     * @param clientId Application's unique client ID
     ...
     * @param accessToken Access token for user authorization
     * @param newIdentityPrimary True if the new identity should be the user's primary identity,false otherwise
     * @return
     */
    public static AuthorizationRequest associateIdentityRequest(String url, String clientId, String openIdProviderUrl, String accessToken, boolean newIdentityPrimary) {
        ...
        return new AuthorizationRequest(requestUrl, clientId).forceNewRequest(true);
    }
